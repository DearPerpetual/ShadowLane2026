...
class Runner(object):
    ...
    def attack(self):
        a_json_path = "/tusimple/clips-shadow.json"
        b_json_path = "/tusimple/clips-clean.json"
        output_path = "/tusimple/identity_list-pair.txt"
        generate_identity_list(a_json_path, b_json_path, output_path)
        format_and_replace_txt(output_path)
        identity_list = get_lfw_list(output_path)
        self.net.eval()
        lfw_test_attack(self.net, identity_list, batch_size=5)
